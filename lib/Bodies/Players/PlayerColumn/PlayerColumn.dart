...
class _PlayerColumn extends State<PlayerColumn> {
  ...
  List<Widget> buildDropDownButtonList(
    ...
    for (int i = 0; i < 4; i++) {
      buttonList.add(Container(
        ...
        child: DropdownButton(
          hint: Text('Player ' + (i + 1).toString()),
          items: choiceList,
          value: selectedNames[i],
          isExpanded: true,
          onChanged: (selectedName) {
            parseSelection(selectedName.toString(), i);
          },
        ),
      ));
    }
  ...
  void parseSelection(String selectedName, int playerNum) {
    ...
    setState(() {
      selectedNames[playerNum] = setVal;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> buttonList = buildDropDownButtonList(choiceList);
